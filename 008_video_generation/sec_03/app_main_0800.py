...
def save_to_csv(df):
    ...
    if file_path:
        ...
        return f'파일을 저장했습니다. 저장 경로는 다음과 같습니다. \n {file_path.name}\n이 플레이리스트의 음원을 내려받겠습니까?', file_path.name
...
def select_mp4_files_and_generate_playlist_video(csv_file):
    ...
    return generate_video_using_mp4(csv_file, video_files)
...
def save_playlist_as_csv(playlist_csv):
    ...
    return f'저장에 실패했습니다. \n 실패한 내용은 다음과 같습니다. {playlist_csv}', None
...
def ask_to_gpt_35_turbo(messages, functions, model='gpt-3.5-turbo', temperature=0.1):
    ...
    response = openai.ChatCompletion.create(
        ...
        temperature=temperature,  # 출력 다양성 조절을 위한 옵션
        messages=messages,  # 이전 메시지와 사용자 입력을 포함하는 메시지 목록
        functions=functions,
        function_call="auto",  # auto is default, but we'll be explicit
    )

    response_message = response.choices[0].message  # 응답 중 첫 번째 응답의 내용 반환
...
def main():
    ...
    messages = [
        {'role': 'system', 'content': """
         ...
         - After downloading the mp3 files in the playlist, you can generate a playlist video using created images or MP4 video files. You should ask the users which option to choose. 
         - After generating the video, you can say goodbye to the users. 
         """}, 
    ]
    ...
    functions = [
        {
            ...
            "parameters": {
                ...
                "properties": {
                    "playlist_csv": {
                        "type": "string",
                        "description": "A playlist in CSV format seperated by ';'.  It must contains a header and the release year foramt should be 'YYYY'. The CSV format must start with a new line. The header of the CSV file must be in English and it should be formatted as follows: 'Title;Artist;Released'.",
                    },
                },
        ...
        },
        {
            ...
            "description": "Generate a playlist video using MP4 video files. This function can be used only after MP3 files have been downloaded.",
            "parameters": {
                "type": "object",
                "properties": {
                    "csv_file": {
